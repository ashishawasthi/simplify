...
JavaScript SDK** 12.19.0 by Google (app, auth, firestore, functions, storage),
downloaded from `https://www.gstatic.com/firebasejs/12.19.0/` by
`tools/vendor-firebase.mjs`, which only rewrites their gstatic import paths to
relative ones (SHA-256 sums in that folder's README). Licence: **Apache License,
Version 2.0** (text below; a copy is in that folder). Learners never load it.

## uqr (coach app only)

`public/coach/vendor/uqr/0.1.3/uqr.js` is **uqr** 0.1.3, the QR code encoder
used for the class poster, byte-identical to its npm release. Copyright (c)
Project Nayuki; Copyright (c) 2023 Anthony Fu. Licence: **MIT** — the full text
is in `public/coach/vendor/uqr/0.1.3/LICENSE.txt`.
