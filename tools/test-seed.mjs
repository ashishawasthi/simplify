...
// tools/seed-institutions.mjs (with the sample in
// tools/fixtures/test-institutions-sample.json — made-up entries, never
// seeded anywhere real) and tools/migrate-coaches.mjs.
//
...
async function runInside() {
  ...
  const SAMPLE = join(ROOT, "tools", "fixtures", "test-institutions-sample.json");
  ...
  let r = tool("seed-institutions.mjs", "--file", SAMPLE, "--dry-run");
  check("a dry run says what it would add", [r.status, /new: 5/.test(r.stdout)], [0, true]);
  check("... and writes nothing", Object.keys(await institutions()).length, 0);
  r = tool("seed-institutions.mjs", "--file", SAMPLE);
  let all = await institutions();
  check("the first run adds every entry", [r.status, Object.keys(all).length], [0, 5]);
  check("... active, with its fields (no source)", Object.keys(all["sample-society-north"]).sort(), ["active", "area", "name", "org", "type", "updatedAt"]);
  check("... all active", Object.values(all).every((i) => i.active === true), true);
  check("... an empty area stays empty", all["sample-home"].area, "");

  // the admin retires one and adds one of their own
  await fs.commit([
    { path: "institutions/sample-school", data: { active: false, updatedAt: SERVER_TIME }, mask: ["active", "updatedAt"] },
    { path: "institutions/admins-own", data: { name: "Admin's own", org: "Other", type: "", area: "", active: true, updatedAt: SERVER_TIME } },
  ]);
  const before = all["sample-society-east"].updatedAt.getTime();
  r = tool("seed-institutions.mjs", "--file", SAMPLE);
  all = await institutions();
  check("a second run changes nothing", [r.status, /new: 0/.test(r.stdout), /changed: 0/.test(r.stdout), /unchanged: 5/.test(r.stdout)], [0, true, true, true]);
  check("... keeps what the admin retired retired", all["sample-school"].active, false);
  ...
  check("... and does not touch an unchanged entry", all["sample-society-east"].updatedAt.getTime(), before);

  // the file corrects a name
  const { readFileSync } = await import("node:fs");
  const fixed = JSON.parse(readFileSync(SAMPLE, "utf8")).map((e) => (e.id === "sample-school" ? { ...e, name: "Sample School (Main)" } : e));
  const tmp = join(process.env.TMPDIR, "fixed.json");
  writeFileSync(tmp, JSON.stringify(fixed));
  r = tool("seed-institutions.mjs", "--file", tmp);
  all = await institutions();
  check("a changed entry is updated", [r.status, all["sample-school"].name], [0, "Sample School (Main)"]);
  check("... and still retired", all["sample-school"].active, false);

  // a bad file writes nothing
  writeFileSync(tmp, JSON.stringify([...fixed, { id: "bad id!", name: "X", org: "Y" }, { id: "sample-home", name: "Twice", org: "Y" }, { id: "no-org", name: "Z", org: " " }]));
  r = tool("seed-institutions.mjs", "--file", tmp);
  check("a bad file is refused with every problem", [r.status, (r.stderr.match(/entry \d+/g) ?? []).length], [1, 3]);
  check("... and nothing is written", Object.keys(await institutions()).length, 6);

  console.log("— migrate-coaches");
  const past = new Date("2026-09-01T02:00:00Z");
  await fs.commit([
    { path: "coaches/old1", data: { name: "Old One", org: "School", note: "", email: "old1@example.com", createdAt: past } },
    { path: "coaches/old2", data: { name: "Old Two", org: "School", note: "", email: "old2@example.com", createdAt: past } },
    { path: "coaches/new1", data: { name: "New", note: "", institutions: ["sample-home"], email: "new1@example.com", status: "declined", createdAt: past } },
  ...
  check("the old org is kept", c.old1.org, "School");
  r = tool("migrate-coaches.mjs", "--apply");
