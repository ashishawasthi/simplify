...
//
//   node tools/shoot-guide.mjs yes        (one scene per run: yes, no, picker)
//
...
// showing a "Cancel" button for a while after the button was gone. Driving
// the real app means a regenerated shot cannot disagree with the shipped
// markup. Take them once a change has been tested, not before.
//
// mic-on-keyboard.png is not a scene: it is a real Android photo, because the
// keyboard belongs to the phone and no headless capture can include it.
//
...
// Fill the screen the way a user would: setting a box and firing "input"
// runs the same handler as a keystroke. Blur last, so no focus ring ends up
// in the picture.
const fill = (money, prices) => `
  const set = (el, v) => { el.value = v; el.dispatchEvent(new Event('input', { bubbles: true })); };
  set(document.getElementById('money-input'), '${money}');
  ${prices.map((price, i) => `
    ${i > 0 ? "document.getElementById('add-item').click();" : ""}
    set([...document.querySelectorAll('#items-list input')].at(-1), '${price}');`).join("")}
  document.activeElement.blur();
`;

const SCENES = {
  // 375x812 at 2x is the 750x1624 the guide's <img>s declare.
  yes: {
    out: "img/guide/screen-yes.png",
    width: 375,
    height: 812,
    setup: fill("50", ["4.50", "12.90"]),
    // Cheap guard against capturing a page that silently failed to set up.
    expect: `document.getElementById('result-subline').textContent === 'Money left: $32.60'`,
  },
  no: {
    out: "img/guide/screen-no.png",
    width: 375,
    height: 812,
    setup: fill("10", ["4.50", "12.90"]),
    expect: `document.getElementById('result-subline').textContent === 'You need $7.40 more'`,
  },
  picker: {
    ...
    setup: `
      ...
      const tap = (what) => document.querySelector('[aria-label^="Add ' + what + '"]').click();
