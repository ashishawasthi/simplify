...
| **Push notifications** | Web Push needs a server and a subscription per device; learner devices check for a new page on opening and at most every 10 minutes | A push service, subscriptions stored per device, and a permission prompt the audience may not manage |
