...
- **Fixed words play a recorded clip.** Every sentence a card can show on its own — each card here and in I need, each Hurts answer, "I want: <a picture's words>", the two second lines — has a clip in one clear Chirp 3 HD voice (Indian English, the closest Google offers to Singapore English), kept on the device. A line with no clip (a stop name, words typed into I want, another language) is read by the device's own `speechSynthesis` voice; one line is never half of each. The clips, the voice and the playback rules are in [the recorded voice](/platform/voice.md).
