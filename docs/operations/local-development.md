...
| Test | Covers | Needs |
|---|---|---|
| `test-assets.mjs` | `ASSETS` in `public/sw.js` and the pages' modulepreload lists are what `tools/stamp.mjs` writes now (every file under `public/` bar the coach app and the few it names, such as `og-card.png`, with its revision; pages by clean URL); and, in a sandbox, the worker's install: byte-for-byte copies, only changed files fetched, stale copies and 404s refused, an interrupted install resumed | Node |
...
A new or changed image under `public/` needs `node tools/stamp.mjs` afterwards.

## Helper scripts

| Script | What it does |
|---|---|
| `node tools/stamp.mjs [--check]` | Writes the generated parts of `public/`: `ASSETS` in `public/sw.js` (every file the learner app keeps offline, with its revision) and the modulepreload lists in `public/index.html` and `public/coach/index.html`. Run it after any change under `public/`; `--check` (run by `test-assets.mjs`) exits 1 if anything is stale. See [offline and updates](/platform/offline-and-updates.md#the-stamp-rule). |
| `node tools/vendor-firebase.mjs` | Downloads the pinned Firebase JS SDK modules (`FIREBASE`, currently 12.19.0: app, auth, firestore, storage, functions) from gstatic and `uqr` (`UQR`, 0.1.3) from jsdelivr into `public/coach/vendor/`, rewriting the SDK's absolute imports to relative ones, with licences and a README of source URLs and SHA-256 sums. The coach CSP is `script-src 'self'` (plus `apis.google.com`), so nothing else loads them. To upgrade: change the version, run it, update the import paths in `public/coach/js/cloud.js` or `public/coach/js/qr.js`, delete the old folder, and `node tools/stamp.mjs` (the coach page's modulepreload list). Hosting serves `/coach/vendor/**` as immutable, so never change a deployed version's folder in place. |
