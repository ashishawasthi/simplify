...
| 2026-09-24 | The service worker's `ASSETS` carries **each file's revision** (a hash of its bytes, written by `tools/stamp.mjs`); the cache is named after them, and an install **fetches only files whose revision changed**, keeps nothing whose bytes don't match, and resumes when cut short. **No `CACHE` number to bump** | A release cost every phone all its HTML, JS and CSS plus ~330 revalidations, which on a slow connection delayed the new version; a forgotten or conflicting `CACHE` bump could not be caught by a test, a stamp can. The hash check also closes the stale-edge risk of 2026-08-12 for good. See [offline and updates](/platform/offline-and-updates.md) |
| 2026-09-24 | Each app page lists its modules as **`<link rel="modulepreload">`** (written by `tools/stamp.mjs`), and the coach app's **vendored libraries are cached as immutable**; still no bundler | A first visit found the learner app's modules one import level at a time, eight deep: on "Fast 3G" the menu took ~3.7 s, now ~2.2 s. The pages stay separate as they are — the guide is already one small page per topic, and splitting the tools out of `index.html` (5 KB compressed) would cost a page load per tool and break instant switching offline |
