...
| 2026-09-26 | A new page reaches open screens by **push, not polling**: while Simplify is on screen, a device streams its class's signal from a **Realtime Database** (`EventSource`, no SDK), which a **Firestore trigger** (`classSignal`) keeps as `{ at, force }`; the 10-minute check on the menu is gone | The owner: prefer Firebase push, with no breach of privacy for minors. Of the Firebase ways, only this one needs no device ID, no stored token and no SDK; a trigger keeps the signal right after every publish, undo, take-down or pause ([security](/platform/security.md#pushing-a-page-to-open-screens-the-privacy-analysis)) |
| 2026-09-26 | **My class shows a new page at once**, even mid-read, from its first screen (it used to wait for the next visit) | The owner: learners with the class page open should see a pushed page immediately |
| 2026-09-26 | A coach can publish with **Show it now on open screens** — off by default, off again after each publish — which opens My class at once on every screen that has Simplify open, whatever it is showing (a running Wait keeps counting; a playing video stops). "Open" means on screen, or back on screen within 5 minutes of the push; an app opened afterwards is never forced | The owner's request and choices; the stream is closed in the background, so a device that is not on screen is not reached |
| 2026-09-26 | Coaches see **no delivery counts** ("reached 12 screens"): learner devices never write anything | Counting would need every learner device to write an "I am here" marker — online-presence data about minors |
