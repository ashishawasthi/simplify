...
- **Silent by default.** No ticking or alarm sounds. Any sound is soft, optional, set by an adult, and starts from a
  tap (Wait's one chime). Speech happens only on a tap, in one recorded clear Indian English voice for a card's fixed
  words and the device's own voice for words someone typed (output only — the microphone stays off for the whole site;
  see [the recorded voice](/platform/voice.md)).
...
- **No account, no analytics, no tracking.** Students never log in, by design: there is no student account, ID or
  ...
  anything only after the learner taps a video, and the guide's videos load from Google only when someone opens one. See [security](/platform/security.md).
