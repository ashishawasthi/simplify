...
- The learner app is plain HTML, CSS and ES modules served as they are from `public/` by Firebase Hosting. There is no framework, no bundler, no transpiler and no npm dependency: `public/index.html` loads `/css/styles.css`, one stylesheet per tool from `public/css/tools/`, and a single `<script type="module" src="/js/app.js">`. Every other module is reached by a static `import`, and is also named in a generated `<link rel="modulepreload">` list in `<head>`, so a first visit asks for all of them at once rather than one import level at a time ([offline and updates](/platform/offline-and-updates.md#the-modulepreload-lists)). The only generated parts of `public/` — that list, the coach page's, and the revisions in `public/sw.js` — are written by `node tools/stamp.mjs`, which is not a build: the files are still served as they are.
...
6. `node tools/stamp.mjs`: the new files into `ASSETS` in `public/sw.js`, with their revisions, and the new modules into `index.html`'s modulepreload list. `node tools/test-assets.mjs` fails if it was forgotten. See [offline and updates](/platform/offline-and-updates.md#the-stamp-rule).
...
8. Only after it has been tested on an Android phone and an iPad: its guide page `public/guide/<id>.html` (then `node tools/stamp.mjs`), its screenshots via `tools/shoot-guide.mjs`, and the `guide` field on its registry entry. See [release and deploy](/operations/release-and-deploy.md).
