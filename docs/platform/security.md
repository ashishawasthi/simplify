...
| `isActiveCoach()` | `coaches/{uid}` exists and its `suspended` is not `true` |
| `isListedCoach(code)` | an active coach whose uid is in `classCoaches/{code}.uids` (the class itself may be paused) |
...
- **Coaches** reach only their own profile and requests, and only the classes they are listed for while their profile is not suspended. Publishing, pages and the picture shelf also need the class to be active. A coach creates their own profile (the email must be the signed-in one) and may later change only `name`, `org`, `note`; a request must be their own, `pending`, and well-formed.
- **Admins** list coaches, requests, classes and usage; set only `suspended` on a coach; decide a pending request once; create, pause and edit classes (including taking a page down, `latest: null`); write `classCoaches` (at most 50 uids); set `config/limits` within fixed ranges; delete pages, pictures and videos.
...
- **Stamps**: the times a coach writes (a profile's or request's `createdAt`, a page's `createdAt` / `updatedAt` / `publishedAt`, a published `latest.publishedAt`, a picture's `createdAt`) and an admin's `decidedAt` must equal `request.time`, and the matching `createdBy`, `updatedBy`, `publishedBy` and `decidedBy` must be the caller.
- Sizes: class name ≤ 30, page and `latest` markdown ≤ 20,000 characters, titles ≤ 80, picture words ≤ 80, pictures at most 1600 × 1600.

## Storage rules

`storage.rules` denies by default, and nothing may be listed. Storage rules may read at most two Firestore documents per request, so its `isClassCoach(code)` checks only that the caller is in `classCoaches/{code}.uids` and not suspended; whether the class is active is left to `firestore.rules`, which decides whether a picture can go on the shelf — and only shelf pictures are ever used. `isAdmin()` is the same `admins/{uid}` check.

| Path | Read | Write |
|---|---|---|
| `classes/{code}/pictures/{id}.jpg` | anyone, by the exact path (`id` matching `isId`) | a class's coach may create only: `image/jpeg`, under 2 MB, never over an existing file (`resource == null`, a new id per upload); the coach or an admin may delete |
...
3. `coaches/{uid}` exists and is not suspended, the class exists and `classCoaches/{code}.uids` includes the uid (`permission-denied`);
