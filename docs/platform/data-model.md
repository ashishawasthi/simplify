...
| `adminLog/{id}` | `action` (one of `coach-approved`, `coach-declined`, `coach-pending`, `coach-suspended`, `coach-unsuspended`, `join-approved`, `join-declined`, `coach-removed`, `coach-added`, `coach-place-added`, `institution-added`, `institution-edited`, `institution-retired`, `institution-restored`, `class-suspended`, `class-restored`, `page-taken-down`, `page-put-back`, `limits-changed`), `adminUid` (= the signed-in admin), `adminEmail` (= their Google sign-in email), `at` (= request time), `note` (≤ 500; **≥ 10 characters for `coach-approved` and `join-approved`**: how the admin checked); as they apply `coach`, `coachName`, `coachEmail`, `request`, `classCode`, `className`, `institution`, `institutionName`, `detail` (≤ 200) — names copied in, so the history reads right after a rename | admins (`get` and list) | admins create, in the same commit as the action; **nobody updates or deletes** (the owner can, in the console). A decision's entry and the decision need each other (`getAfter`), so neither is written alone |
