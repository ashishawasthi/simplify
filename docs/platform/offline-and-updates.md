...
- `refreshClass()` reads the class when the push signal says the saved page is not the latest (`class-live.js`, only while the app is on screen), and whenever My class opens (then only "not twice within 15 seconds"). `keepClassMedia()` then downloads the files a new page needs and deletes every other entry — another page's, another class's. The push stream goes to another origin, so the service worker never sees it.
