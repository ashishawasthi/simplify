...
| `#admin`, `#admin/<tab>` | the admin's screen (only when the signed-in user is an admin), in tabs: `waiting` (the default), `coaches`, `classes`, `places`, `history`, `limits` | `screen-admin.js` |
...
`coaches/{uid}`: **name** (required, 60 characters), **Where you work** — up to ten institutions from the admin's list (`institutions/{id}`, see [admin and approvals](/coach/admin-and-approvals.md#institutions)) — **Not in the list? Type the name of your school or centre** (`otherPlace`, 120 characters, spaces tidied; hint "The admin adds it to the list.") and "How can the admin check who you are?" (300), plus the sign-in email, which cannot be changed here. Only the coach and admins can read it. Save waits for a name and a place: at least one institution, or a place typed.
...
- **Later saves** may change only name, note, institutions and `otherPlace` (an empty one is deleted) — whatever the admin decided. Changing either place also writes `institutionsChangedAt` (the rules insist on it), and an approved coach stays approved: the admin trusts the person, and the admin's coach list says "Changed …, after approval". `status`, `decidedAt`, `decidedBy`, `decisionLog`, `decisionMessage`, `suspended` and `suspendLog` belong to the admin.
- **A place not in the list**: the admin adds it to the list from the coach's card, and it replaces `otherPlace` on the profile in the same write (or the admin picks the listed place it really is). Until then an approved coach's **New class** says that the place isn't in the list yet, and that they can make a class there once the admin adds it.
...
`public/coach/js/screen-waiting.js`. Until an admin approves them, a coach can do nothing but read and change their own profile: the rules refuse every class, page, picture, request and upload, and every callable refuses with "The admin has not approved you yet." The screen is calm and says what happens next: the admin checks them — "for example, they may have seen you teach at your school, or they call it" — and may contact them; the page changes by itself when they are approved (they can close it and sign in again later with the same email); then they can make classes or join a colleague's. Below, **What the admin sees** (name, where they work — a typed place marked "(not in the list yet)" — how to check them) and **Change About you**.

A declined coach sees **Not approved**: the admin's message if they wrote one ("The admin says: “Please add a work email I can check.”", from `decisionMessage`), otherwise "The admin has not approved you as a coach. If you think this is a mistake, contact the admin."; then "If something in About you was missing or wrong, change it, then ask the admin again.", **Ask the admin again** (`cloud.askAgain`: `status` back to `pending` with `reappliedAt`; the watched profile turns the screen into Waiting for approval, and the admin sees "Asked again") and the same summary and link. An admin can also approve them later; nothing is deleted.
...
- `node tools/smoke.mjs coach` (`tools/smoke/coach.mjs`): every screen and flow in real Chrome against a stand-in `cloud.js` — including About you's picker, Waiting for approval turning into My classes when the admin approves, Not approved, a profile from before institutions, making a class, and the admin's approvals (with how they checked), history, classes and institutions, a place not in the list, and asking again.
