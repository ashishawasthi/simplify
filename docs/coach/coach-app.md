...
The Firebase JS SDK **12.19.0** (app, auth, firestore, storage, functions) and **uqr 0.1.3** (the QR code) are vendored into `public/coach/vendor/` by `node tools/vendor-firebase.mjs`, which pins both versions, rewrites the SDK's gstatic import specifiers to relative ones, drops source-map comments and writes each folder's licence and a README with SHA-256 sums. Upgrading means changing the version there, running it, updating the import paths in `public/coach/js/cloud.js` or `public/coach/js/qr.js`, deleting the old folder, and running `node tools/stamp.mjs`. Hosting serves `/coach/vendor/**` as `immutable` for a year (the folders are named by version), so a coach's browser keeps the SDK without asking again; a deployed version's folder is never changed in place. `public/coach/index.html` lists every module `main.js` reaches, the SDK included, as `<link rel="modulepreload">`, so they are all asked for as the page arrives ([offline and updates](/platform/offline-and-updates.md#the-modulepreload-lists)). The licences are listed in `THIRD_PARTY_NOTICES.md`. The learner app does not load the SDK; it reads a class with one plain HTTPS request (see [My class](/learner/my-class.md)).
